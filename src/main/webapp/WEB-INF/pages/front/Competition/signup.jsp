...
<body>
    ...
    <!-- Page Header End -->
    <div class="modal fade user-select-none" id="exampleModal" tabindex="-1"
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary"
                        onclick="javascript:history.back(-1);">No</button>
    ...
    <div class="container text-white">
        <form:form action="${contextRoot}/competition/signup/create" modelAttribute="signup">
            <div class="input-group">
                <div class="d-flex flex-column">
                            ...
                            <c:otherwise>
                                <div class="col">
                                    ...
                                    <form:select class="form-select" path="competitionId"
                                        id="competitionId">
                                        <form:option value="">-- 請選擇一個活動 --</form:option>
                                        <c:forEach items="${compList}" var="comp">
                                            <form:option value="${comp.id}">
                                                ${comp.mandarinName}（${comp.englishName}）；
                                                <c:choose>
                                                    <c:when test="${comp.singleOrCrew == \" S\"}">個人戰
                                                    </c:when>
                                                    <c:when test="${comp.singleOrCrew == \" C\"}">團體戰
                                                    </c:when>
                                                    <c:otherwise>未定</c:otherwise>
                                                </c:choose>
                                                ；報名費&nbsp;${comp.fee}&nbsp;元
                    ...
                    <fieldset class="mt-3 mb-3 p-3">
                        ...
                        <div class="col-12">
                            <form:radiobutton name="status" class="form-check-input" path="status" id="paid"
                                value="已繳費" label="是" required="true" />
                            <form:radiobutton name="status" class="form-check-input" path="status"
                                id="notpaid" value="未繳費" label="否" required="true" />
                        </div>
                    </fieldset>
